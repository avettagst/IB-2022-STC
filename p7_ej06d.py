...
L = 500 #largo de la fibra en km
d = 50#cant de pasos para el vector z
z = np.linspace(0, L, d)



n = 16384 #cantidad de muestras
dt = 0.5 #en picosegundos
...
ancho_teo = []
for pos in z:
    Azf = A0f * np.exp((1/6)*1j*b3*pos*(2*np.pi*f)**3) * np.exp(0.5*1j*b2*pos*(2*np.pi*f)**2)
    Azt = sc.fft.ifft(Azf)
